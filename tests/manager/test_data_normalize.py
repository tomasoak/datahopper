...
from datahopper.manager.normalize import (
    clean_string,
...
def test_clean_string():
    ...
    assert clean_string(string_special, especial_char=True) == string_special_cleaned
...
def test_rename_column():
    ...
    df_renamed = pd.DataFrame(columns=["id", "municipality", "country", "continent"])
...
def test_drop_rows_missing_values():
    ...
    df_cleaned = pd.DataFrame(
        columns=["id", "name", "age", "gender"],
        data=[[1, "Alex", 32, "Non-Binary"], [3, "Austin", 57, "Male"]],
    )

    assert all(
        drop_rows_missing_values(df, df.columns).sort_index(inplace=True) == df_cleaned
    )
