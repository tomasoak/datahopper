...
class Chart:
    ...
    def histogram(self, title: str, label: str, yaxis_title: str):
        fig = px.histogram(
            self.df[self.df[self.agg_col] > 0],
            x=self.agg_col,
            marginal="box",
            title=f"{title}",
            labels={self.agg_col: label},
            height=400,
            width=900,
        )
        fig.update_layout(
    ...
    def bar_year(self, title: str, label: str):
        ...
        df_year = (
            ...
            .agg({self.agg_col: "sum"})
            .sort_values(self.agg_col, ascending=False)
            .reset_index()
        )
        df_year["category"] = [str(i) for i in df_year.index]
        df_year["color"] = "rgb(189,189,189)"
        df_year.loc[df_year["category"] == "0", "color"] = "rgb(255,106,95)"
        df_year.loc[df_year["category"] == "1", "color"] = "rgb(120,120,120)"
        colors = df_year["color"].values
        ...
        fig = px.bar(
            ...
            color="category",
            hover_name="YEAR",
            hover_data={"category": False, "YEAR": False},
    ...
    def bar_year_uf(self, title: str, label: str):
        """Vertical bar chart"""
        df_per_year_uf = (
            self.df[self.df["YEAR"] >= 2013]
            .groupby(["YEAR", "SIGLA_UF"], as_index=False)
            .agg({self.agg_col: "sum"})
        )
        df_per_year_uf = df_per_year_uf.assign(
            UF_YEAR=lambda x: x["SIGLA_UF"] + " - " + x["YEAR"].astype(str)
        )
        ...
        fig = px.bar(
            ...
            hover_name="UF_YEAR",
            hover_data={"YEAR": False, "SIGLA_UF": False},
            height=500,
            width=900,
            labels={"YEAR": "", self.agg_col: label, "SIGLA_UF": "State"},
        )
    ...
    def bar_mun_uf_total(self, agg_method: str, title: str, label: str):
        ...
        fig = px.bar(
            ...
            title=f"{title}",
            hover_name="MUN_UF",
            hover_data={"category": False, "MUN_UF": False},
            height=600,
            width=800,
    ...
    def subplot_hist(self, title: str):
        ...
        trace0 = go.Histogram(
            x=df[df["YEAR"] == 2020][self.agg_col],
            autobinx=True,
            name="2020",
            marker_color="rgb(255,106,95)",
        )
        trace1 = go.Histogram(
            x=df[df["YEAR"] == 2019][self.agg_col],
            autobinx=True,
            name="2019",
            marker_color="rgb(120,120,120)",
        )
        trace2 = go.Histogram(
            x=df[df["YEAR"] == 2018][self.agg_col],
            autobinx=True,
            name="2018",
            marker_color="rgb(189,189,189)",
        )
        trace3 = go.Histogram(
            x=df[df["YEAR"] == 2017][self.agg_col],
            autobinx=True,
            name="2017",
            marker_color="#EE7A68",
        )
        trace4 = go.Histogram(
            x=df[df["YEAR"] == 2016][self.agg_col],
            autobinx=True,
            name="2016",
            marker_color="#604693",
        )
        trace5 = go.Histogram(
            x=df[df["YEAR"] == 2015][self.agg_col],
            autobinx=True,
            name="2015",
            marker_color="rgb(189,189,189)",
        )
        trace6 = go.Histogram(
            x=df[df["YEAR"] == 2014][self.agg_col],
            autobinx=True,
            name="2014",
            marker_color="#330C73",
        )
        trace7 = go.Histogram(
            x=df[df["YEAR"] == 2013][self.agg_col],
            autobinx=True,
            name="2013",
            marker_color="#EB89B5",
        )
